...
func checkText(str string, location string, comment_author string) []Detection {
	...
	if !whitelisted_logins[strings.ToLower(comment_author)] {
		...
		patterns := []string{
			...
			"i didn't receive", "binance", "kucoin", "bitget", "coinbase wallet", "bybit", "has not arrived", "has not yet arrived", "hasn't arrived",
			...
			"made a withdrawal", "receive my withdrawal", "can not claim", "lost payment", "i bought", "mistakenly wrong token", "send by mistake", "wrong address", "withdraw issue",
