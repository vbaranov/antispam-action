...
func checkText(str string, location string, comment_author string) []Detection {
	...
	if !whitelisted_logins[strings.ToLower(comment_author)] {
		...
		patterns := []string{
			...
			"i didn't receive", "binance", "kucoin", "coinbase wallet", "bybit", "transaction has not arrived",
			...
			"missing eth", "see tokens in metamask", "can not swap", "i lose", "i buy", "send to wrong", "sending money", "claim failed",
		}
