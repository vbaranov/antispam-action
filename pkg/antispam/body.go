...
func checkText(str string, location string, comment_author string) []Detection {
	...
	if !whitelisted_logins[strings.ToLower(comment_author)] {
		...
		patterns := []string{
			...
			"did not arrive", "didint arrive", "didn't get the token", "token not received",
			...
			"wrong network", "wrong chain", "wrong chain id", "wrong wallet address", "wrong blockchain address", "wrong address", "wrong deposit", "wrong transaction", "wrong send", "send to wrong", "send by mistake", "by mistake sent", "mistakenly send", "mistakenly sent", "transaction mistake", "wrongly transfer",
			"withdraw not received", "never received", "withdraw issue", "withdrawal not delivered", "withdrawals from my", "pending withdrawal", "made a withdrawal", "receive my withdrawal", "received my withdrawal", "withdraw money", "withdraw problem", "withdrawal expired", "withdrawal money",
			...
			"wrong memo", "missing memo", "missing tag", "transfer was rejected", "investment",
			...
			"not deposited", "get my funds", "receive my funds", "i lose", "i buy", "sending money", "my money", "sent money", "money sent", "lost payment", "i bought",
			...
			"stole my tokens", "money never came", "accidentally deposited", "hack my wallet", "item havent received", "wallet is not open", "wallet is compromised", "never arrived",
			"accidently sent",
		}
