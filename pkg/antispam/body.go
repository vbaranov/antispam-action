...
func checkText(str string, location string, comment_author string) []Detection {
	...
	if !whitelisted_logins[strings.ToLower(comment_author)] {
		...
		patterns := []string{
			...
			"transaction didn't go", "i was debited", "crypto didn’t transfer",
			...
			"not receive", "haven't received", "has not arrived", "has not yet arrived", "hasn't arrived", "not yet received", "not been received", "i don't receive", "i didn't receive", "didn't received",
			"token did not arrive", "didn't get the token", "token not received",
			...
			"not deposited", "get my funds", "i lose", "i buy", "sending money", "my money", "lost payment", "i bought", "mistakenly wrong token",
		}
