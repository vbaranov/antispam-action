...
func checkText(str string, location string, comment_author string) []Detection {
	...
	if !whitelisted_logins[strings.ToLower(comment_author)] {
		...
		patterns := []string{
			...
			"withdraw not received", "never received", "withdraw issue", "withdrawal not delivered", "pending withdrawal", "made a withdrawal", "receive my withdrawal", "received my withdrawal", "withdraw money", "withdraw problem",
			...
			"successful transaction but not delivered", "pending transaction", "send to etherum", "send plume",
		}
